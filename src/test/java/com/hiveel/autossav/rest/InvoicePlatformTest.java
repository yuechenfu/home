...
public class InvoicePlatformTest {
    ...
    public void count() throws Exception {
        String result = mvc.perform(MockMvcRequestBuilders.get("/mg/invoicePlatform/count").header("authorization", "Bearer 12345678")).andReturn().getResponse().getContentAsString();
    ...
    public void find() throws Exception {
        String result = mvc.perform(MockMvcRequestBuilders.get("/mg/invoicePlatform").header("authorization", "Bearer 12345678")).andReturn().getResponse().getContentAsString();
        Rest<List<InvoicePlatform>> rest = Rest.fromJson(result, BasicRestCode.class, new TypeToken<Rest<List<InvoicePlatform>>>(){}.getType());
        Assert.assertEquals(BasicRestCode.SUCCESS, rest.getCode());
